...
public class TowerRange : MonoBehaviour
{
    ...
    void OnTriggerEnter2D(Collider2D collision) {
        ...
        if (collision.CompareTag("Monster") && !isBarracks) {
            GameManager.Instance.Towers[GridPosition].MonsterInRange(collision.GetComponent<Monster>());
        }
    }

    void OnTriggerExit2D(Collider2D collision) {
        if (collision.CompareTag("Monster") && collision.GetComponent<Monster>().Equals(GameManager.Instance.Towers[GridPosition].Target) && !isBarracks) {
